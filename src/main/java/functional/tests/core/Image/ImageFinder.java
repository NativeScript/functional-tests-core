...
public class ImageFinder {
    ...
    public ImageFinder(String setRotation) {
    ...
    public Point[] findImage(String object_filename_nopng, String scene_filename_nopng, double tolerance) {
        ...
        if (jsonObject == null) {
            Log.error("ERROR: Json file couldn't be processed. ");
        ...
        if ((objList.size() < 4) || (sceneList.size() < 4)) {
            Log.error("Not enough matches found. ");
            return null;
        }
        ...
        Log.info("90 Degree Rotation: " + rotationAngle);
        ...
        Log.info("Image found at coordinates: " + (int) center.x + ", " + (int) center.y + " on scene.");
        ...
        Log.info("Top left original: " + points[0].x + ", " + points[0].y);
        Log.info("Top right original: " + points[1].x + ", " + points[1].y);
        Log.info("Bottom right original: " + points[2].x + ", " + points[2].y);
        Log.info("Bottom left original: " + points[3].x + ", " + points[3].y);
        Log.info("Center: " + points[4].x + ", " + points[4].y);
    ...
    private boolean checkFoundImageSizeRatio(double initial_height, double initial_width, Point top_left, Point top_right, Point bottom_left, Point bottom_right, double initial_ratio, double found_ratio1, double found_ratio2, double tolerance) {
        ...
        if ((round(found_ratio1 / initial_ratio, 2) > (1 + tolerance)) || (round(initial_ratio / found_ratio2, 2) > (1 + tolerance))
            ...
            Log.error("Size of image found is incorrect, check the ratios for more info:");
            Log.info("Initial height of query image: " + initial_height);
            Log.info("Initial width of query image: " + initial_width);
            Log.info("Initial ratio for query image: " + initial_height / initial_width);

            Log.info("Found top width: " + (top_right.x - top_left.x));
            Log.info("Found bottom width: " + (bottom_right.x - bottom_left.x));

            Log.info("Found left height: " + (bottom_left.y - top_left.y));
            Log.info("Found right height: " + (bottom_right.y - top_right.y));
            Log.info("Found ratio differences: " + round(found_ratio1 / initial_ratio, 1) + " and " + round(initial_ratio / found_ratio2, 1));
            return true;
        }
        return false;
    }
    ...
    private boolean checkFoundImageDimensions(Point top_left, Point top_right, Point bottom_left, Point bottom_right, double tolerance) {
        ...
        Log.info("Height and width ratios: " + height_ratio + " and " + width_ratio);

        if ((height_ratio < (1 - tolerance)) || (height_ratio > (1 + tolerance)) || (width_ratio < (1 - tolerance)) || (width_ratio > (1 + tolerance))) {
            Log.info("Height and width ratios: " + height_ratio + " and " + width_ratio);
            Log.error("Image found is not the correct shape, height or width are different on each side.");
            return true;
        } else {
            return false;
        }
    ...
    private String runAkazeMatch(String object_filename, String scene_filename) throws InterruptedException, IOException {
        ...
        if (!file.exists()) {
            Log.error("ERROR: Image recognition with Akaze failed. No json file created.");
            return null;
        } else {
            return jsonFilename;
        }
